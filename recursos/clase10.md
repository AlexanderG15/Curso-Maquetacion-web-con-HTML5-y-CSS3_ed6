# Clase 10

### Cajas

- [box-shadow en MDN](https://developer.mozilla.org/es/docs/Web/CSS/box-shadow)

### Posición

- [float en MDN](https://developer.mozilla.org/es/docs/Web/CSS/float)
- [position en MDN](https://developer.mozilla.org/es/docs/Web/CSS/position)
- [z-index en MDN](https://developer.mozilla.org/es/docs/Web/CSS/z-index)
- [overflow en MDN](https://developer.mozilla.org/es/docs/Web/CSS/overflow)
- [clear en MDN](https://developer.mozilla.org/es/docs/Web/CSS/clear)

### Visibilidad

- [display en MDN](https://developer.mozilla.org/es/docs/Web/CSS/display)
- [visibility en MDN](https://developer.mozilla.org/es/docs/Web/CSS/visibility)
- [opacity en MDN](https://developer.mozilla.org/es/docs/Web/CSS/opacity)

### Apoyo

- [VIDEO - CSS Float and Clear Explained - How does CSS float and clear work?](https://www.youtube.com/watch?v=xara4Z1b18I)
